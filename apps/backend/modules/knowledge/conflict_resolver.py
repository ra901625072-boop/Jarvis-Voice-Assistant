    ...
    def _is_contradicting(self, old_content: str, new_content: str) -> bool:
        ...
        if old_match and new_match:
            ...
            if old_val != new_val and len(old_val) > 2 and len(new_val) > 2:
                ...
                old_subj = old_content[:old_content.rfind(old_val)].strip()
                new_subj = new_content[:new_content.rfind(new_val)].strip()
                while True:
                    old_len = len(old_subj)
                    old_subj = re.sub(r"\s*(\b(is|are)\b|=)\s*$", "", old_subj).strip()
                    if len(old_subj) == old_len:
                        break
                while True:
                    new_len = len(new_subj)
                    new_subj = re.sub(r"\s*(\b(is|are)\b|=)\s*$", "", new_subj).strip()
                    if len(new_subj) == new_len:
                        break
