"""
tool_memory.py
--------------
Per-tool performance tracking for JARVIS.

Purpose
-------
JARVIS has 40+ tools.  Some are highly reliable (95%+ success).
Others are fragile (Selenium scraping, OCR, screen vision).

Without tool memory, JARVIS has no way to:
1. Know which tool to prefer when multiple options exist
2. Warn the user when a tool is known to be unreliable
3. Recommend alternatives when a primary tool repeatedly fails

Architecture
------------
ToolMemory stores per-tool stats in the `tool_memory` SQLite table.
It is fed by the automatic wrapper in JarvisToolset.safe_execute()
so every tool call is recorded without any manual instrumentation.

Reliability score
-----------------
Rolling exponential moving average:
  new_score = 0.8 * old_score + 0.2 * (1.0 if success else 0.0)

This weights recent calls more heavily than old ones.
A tool that was bad but improved gradually recovers its score.
"""

import logging
from datetime import datetime
from typing import Optional, Dict, Any, List

logger = logging.getLogger("JARVIS.ToolMemory")

# Tunable
_EMA_ALPHA          = 0.20   # exponential moving average weight for new observations
_LOW_RELIABILITY    = 0.60   # tools below this get flagged
_MIN_CALLS_TO_FLAG  = 3      # don't flag until we have this many calls


class ToolMemory:
    """
    ToolMemory records and retrieves per-tool performance metrics.

    SYSTEM PROMPT:
    Initialize or query ToolMemory to record success/failure outcomes, fetch reliability scores, or check for tool failure warnings.

    SHORT DESCRIPTION:
    Performance analytics tracker that logs execution status, elapsed times, and errors to calculate rolling reliability metrics.

    PROCESS:
    1. Logs tool execution outcome parameters (tool_name, success, execution time, error info).
    2. Calculates a rolling Exponential Moving Average (EMA) reliability score.
    3. Exposes queries to list unreliable tools (scoring below threshold metrics after minimal execution calls).
    4. Formulates diagnostic and cautionary warning prompts for the agent.

    FLOW:
    Caller/SafeExecute -> record() -> rolling EMA calculations -> SQLite UPDATE/INSERT -> Caller
    """

    def __init__(self, memory_manager):
        self.mm    = memory_manager
        self._dbs  = memory_manager.dbs
        self._lock = memory_manager._lock

    # ------------------------------------------------------------------ #
    # Core recording                                                       #
    # ------------------------------------------------------------------ #

    def record(
        self,
        tool_name: str,
        success: bool,
        exec_time_ms: int = 0,
        error: Optional[str] = None,
        context_tag: str = "general"
    ) -> None:
        """
        Record a tool call outcome.  Called automatically by safe_execute().
        """
        import os
        if os.environ.get("JARVIS_E2E_SIM") == "1":
            return
        ts = datetime.now().isoformat()
        try:
            with self._lock:
                existing = self._dbs["conversations"].execute(
                    """SELECT id, success_count, fail_count,
                              avg_exec_time_ms, reliability_score
                       FROM tool_memory WHERE tool_name=? AND context_tag=?""",
                    (tool_name, context_tag),
                ).fetchone()

                if existing:
                    row_id, succ, fail, avg_ms, old_score = existing
                    new_succ  = succ + (1 if success else 0)
                    new_fail  = fail + (0 if success else 1)
                    total     = new_succ + new_fail
                    # Rolling average of execution time
                    new_avg   = int((avg_ms * (total - 1) + exec_time_ms) / total) if total else 0
                    # Exponential moving average reliability
                    new_score = _EMA_ALPHA * (1.0 if success else 0.0) + (1 - _EMA_ALPHA) * old_score
                    new_score = round(new_score, 4)

                    self._dbs["conversations"].execute(
                        """UPDATE tool_memory
                           SET success_count=?, fail_count=?, avg_exec_time_ms=?,
                               reliability_score=?, last_failure_reason=?, last_used=?, updated_at=?
                           WHERE id=?""",
                        (
                            new_succ, new_fail, new_avg, new_score,
                            error if not success else None,
                            ts, ts, row_id,
                        ),
                    )
                else:
                    init_score = 1.0 if success else (1 - _EMA_ALPHA)
                    self._dbs["conversations"].execute(
                        """INSERT INTO tool_memory
                           (tool_name, success_count, fail_count, avg_exec_time_ms,
                            last_failure_reason, last_used, reliability_score, updated_at, context_tag)
                           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)""",
                        (
                            tool_name,
                            1 if success else 0,
                            0 if success else 1,
                            exec_time_ms,
                            error if not success else None,
                            ts,
                            round(init_score, 4),
                            ts,
                            context_tag
                        ),
                    )

                # Lazy commit — no force needed for analytics data
                self._dbs["conversations"].commit()

        except Exception as e:
            # Never let tool tracking break the main execution
            logger.debug(f"ToolMemory.record failed silently: {e}")

    # ------------------------------------------------------------------ #
    # Querying                                                             #
    # ------------------------------------------------------------------ #

    def _apply_decay(self, score: float, last_used_str: str) -> float:
        """Decay reliability difference back to 1.0 baseline exponentially over 30 days."""
        if not last_used_str:
            return score
        try:
            last_used = datetime.fromisoformat(last_used_str)
            elapsed_days = (datetime.now() - last_used).total_seconds() / 86400.0
            import math
            decayed_score = 1.0 - (1.0 - score) * math.exp(-elapsed_days / 30.0)
            return round(decayed_score, 4)
        except Exception:
            return score

    def _get_confidence_indicator(self, total_calls: int) -> str:
        """Return confidence level description based on sample size."""
        if total_calls <= 2:
            return "Very Low (sample size <= 2)"
        elif total_calls <= 5:
            return "Low (sample size <= 5)"
        elif total_calls <= 15:
            return "Medium"
        else:
            return "High"

    def get_reliability(self, tool_name: str, context_tag: str = "general") -> float:
        """Return reliability score (0–1) for a tool, or 1.0 if unknown."""
        with self._lock:
            row = self._dbs["conversations"].execute(
                "SELECT reliability_score, success_count, fail_count, last_used FROM tool_memory WHERE tool_name=? AND context_tag=?",
                (tool_name, context_tag),
            ).fetchone()
            
            # Fallback to general if specific context has no data
            if not row and context_tag != "general":
                row = self._dbs["conversations"].execute(
                    "SELECT reliability_score, success_count, fail_count, last_used FROM tool_memory WHERE tool_name=? AND context_tag='general'",
                    (tool_name,),
                ).fetchone()
                
        if not row:
            return 1.0
        score, succ, fail, last_used = row
        total = succ + fail
        if total < _MIN_CALLS_TO_FLAG:
            return 1.0  # not enough data
        return self._apply_decay(score, last_used)

    def get_tool_stats(self, tool_name: str, context_tag: str = "general") -> Optional[Dict[str, Any]]:
        """Return full stats dict for a tool."""
        with self._lock:
            row = self._dbs["conversations"].execute(
                """SELECT success_count, fail_count, avg_exec_time_ms,
                          reliability_score, last_failure_reason, last_used, context_tag
                   FROM tool_memory WHERE tool_name=? AND context_tag=?""",
                (tool_name, context_tag),
            ).fetchone()
            
            if not row and context_tag != "general":
                row = self._dbs["conversations"].execute(
                    """SELECT success_count, fail_count, avg_exec_time_ms,
                              reliability_score, last_failure_reason, last_used, context_tag
                       FROM tool_memory WHERE tool_name=? AND context_tag='general'""",
                    (tool_name,),
                ).fetchone()
                
        if not row:
            return None
        succ, fail, avg_ms, score, last_err, last_used, ctx = row
        total = succ + fail
        decayed_score = self._apply_decay(score, last_used)
        return {
            "tool_name":       tool_name,
            "context_tag":     ctx,
            "success_count":   succ,
            "fail_count":      fail,
            "total_calls":     total,
            "success_rate":    round(succ / total * 100, 1) if total else 0.0,
            "reliability":     decayed_score,
            "avg_exec_time_ms": avg_ms,
            "last_failure":    last_err,
            "last_used":       last_used,
            "confidence":      self._get_confidence_indicator(total)
        }

    def get_all_tool_report(self) -> str:
        """
        Return a formatted tool performance report for all tracked tools.
        Sorted by reliability ascending (worst first).
        """
        with self._lock:
            rows = self._dbs["conversations"].execute(
                """SELECT tool_name, context_tag, success_count, fail_count,
                          reliability_score, avg_exec_time_ms, last_used
                   FROM tool_memory
                   WHERE (success_count + fail_count) >= ?""",
                (_MIN_CALLS_TO_FLAG,),
            ).fetchall()

        if not rows:
            return "No tool performance data available yet."

        # Decay scores and then sort by reliability ascending
        report_data = []
        for tool, ctx, succ, fail, score, avg_ms, last_used in rows:
            decayed_score = self._apply_decay(score, last_used)
            report_data.append((tool, ctx, succ, fail, decayed_score, avg_ms, last_used))
            
        report_data.sort(key=lambda x: x[4])

        lines = ["Tool Performance Report:"]
        for tool, ctx, succ, fail, score, avg_ms, last_used in report_data:
            total    = succ + fail
            rate     = round(succ / total * 100, 1) if total else 0
            status   = "OK" if score >= _LOW_RELIABILITY else "UNRELIABLE"
            ctx_str  = f" [{ctx}]" if ctx != "general" else ""
            conf_str = self._get_confidence_indicator(total)
            lines.append(
                f"  [{status}] {tool}{ctx_str}: {rate}% success "
                f"({succ}/{total} calls, confidence: {conf_str}, avg {avg_ms}ms, score={score:.2f})"
            )
        return "\n".join(lines)

    def get_unreliable_tools(self) -> List[Dict[str, Any]]:
        """Return tools with reliability below threshold and enough call history."""
        with self._lock:
            rows = self._dbs["conversations"].execute(
                """SELECT tool_name, context_tag, reliability_score, success_count, fail_count, last_used
                   FROM tool_memory
                   WHERE (success_count + fail_count) >= ?""",
                (_MIN_CALLS_TO_FLAG,),
            ).fetchall()

        unreliable = []
        for r in rows:
            decayed_score = self._apply_decay(r[2], r[5])
            if decayed_score < _LOW_RELIABILITY:
                unreliable.append({
                    "tool_name":   r[0],
                    "context_tag": r[1],
                    "reliability": decayed_score,
                    "success":     r[3],
                    "fail":        r[4],
                    "confidence":  self._get_confidence_indicator(r[3] + r[4])
                })
        # Sort by reliability ascending
        unreliable.sort(key=lambda x: x["reliability"])
        return unreliable

    def recommend_caution(self, tool_name: str, context_tag: str = "general") -> Optional[str]:
        """
        Return a caution string if a tool is known to be unreliable,
        or None if the tool is reliable or untracked.
        """
        score = self.get_reliability(tool_name, context_tag)
        if score < _LOW_RELIABILITY:
            pct = round((1 - score) * 100, 1)
            ctx_str = f" in context '{context_tag}'" if context_tag != "general" else ""
            return (
                f"Note: '{tool_name}' has been unreliable recently{ctx_str} "
                f"({pct}% failure rate). Proceed with caution or use an alternative."
            )
        return None
