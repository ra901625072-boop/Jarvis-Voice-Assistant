...
from modules.memory.gate import MemoryGate
from modules.knowledge.conflict_resolver import ConflictResolver
from modules.learning.experience_replay import ExperienceReplay
from modules.planning.goal_memory import GoalMemory
from modules.learning.tool_memory import ToolMemory
from modules.memory.consolidator import MemoryConsolidator
from modules.learning.reflection_engine import ReflectionEngine
...
class MemoryLifecycle:
    ...
    def run_nightly(self) -> None:
        ...
        try:
            # 3.5. Agent Self-Reflection
            from modules.learning.self_reflector import AgentSelfReflector
            with self._lock:
                agent_ids = [r[0] for r in self._dbs["conversations"].execute(
            ...
            reflector = AgentSelfReflector(self.mm)
            for aid in agent_ids:
                reflector.run_for_agent(aid)
        except Exception as e:
            logger.error(f"Agent self reflection failed: {e}")

        try:
            # 3.6. Agent Capability Tracker
            from modules.learning.capability_tracker import AgentCapabilityTracker
            AgentCapabilityTracker(self.mm).run_nightly()
        except Exception as e:
            logger.error(f"Agent capability tracker failed: {e}")

        try:
            # 3.7. Decay Old Success Patterns
            from modules.execution.success_patterns import SuccessLearner
            SuccessLearner(self.mm).decay_old_patterns()
        except Exception as e:
            logger.error(f"Success pattern decay failed: {e}")

        try:
            # 3.8. Summarize Learning Cycle nightly via LearningAgent
            from ai.agents.types import AgentTask
            import uuid
            import asyncio
            from container import ServiceContainer
            c = ServiceContainer.instance()
            learning_agent = c.get_or_none("learning_agent") if c else None
            if learning_agent:
                task = AgentTask(
                    task_id=str(uuid.uuid4()),
                    task_type="summarize_learning_cycle",
                    payload={},
                    origin_agent="memory_lifecycle",
                    target_agent="learning_agent"
                )
                try:
                    loop = asyncio.get_running_loop()
                    loop.create_task(learning_agent.handle(task))
                except RuntimeError:
                    # No running loop, run synchronously
                    asyncio.run(learning_agent.handle(task))
        except Exception as e:
            logger.error(f"Nightly learning cycle summary failed: {e}")
    ...
    def build_context(
        ...
        # Project-specific context if project_name is supplied
        if project_name:
            project_str = self.mm.get_project_context(project_name)
            if project_str and not project_str.startswith("No memories found"):
                sections.append({"priority": 7.5, "text": project_str})
        ...
        if current_query:
            lessons = self._get_relevant_lessons(current_query, project=project_name)
            if lessons:
                sections.append({"priority": 5, "text": f"--- LESSONS LEARNED ---\n{lessons}"})

        # 6.5. Validated Operational Strategies & Invariants
        try:
            with self._lock:
                strat_rows = self._dbs["conversations"].execute(
                    """SELECT name, action_guidance, status FROM strategies
                       WHERE status IN ('validated', 'trusted', 'permanent')
                       ORDER BY utility_score DESC LIMIT 3"""
                ).fetchall()
            if strat_rows:
                strat_str = "--- OPERATIONAL STRATEGIES & INVARIANTS ---\n"
                for s in strat_rows:
                    strat_str += f"[{s[2].upper()}] {s[0]}: {s[1][:250]}\n"
                sections.append({"priority": 5.5, "text": strat_str.strip()})
        except Exception:
            pass

        # 7. Recent reflections (last 3 days)
        reflections = self.mm.get_agent_reflections(days=3, project=project_name)
        if reflections:
    ...
    def _get_relevant_lessons(self, query: str, project: Optional[str] = None) -> str:
        """Fetch lessons relevant to the query from lessons_learned, optionally filtered by project."""
        try:
            ...
            if project and project != "general":
                project_clause = " AND (project = ? OR project = 'general')"
                params.append(project)
                
            with self._lock:
                rows = self._dbs["conversations"].execute(
                    f"""SELECT lesson FROM lessons_learned
                        WHERE ({like_clauses}){project_clause}
    ...
    def run_session_learning(self) -> None:
        """Immediate learning triggered at the end of a user session (no wait for nightly)."""
        logger.info("MemoryLifecycle: starting session-end learning pass...")
        try:
            from modules.learning.reflection_engine import ReflectionEngine
            ReflectionEngine(self.mm).run_daily()
            logger.info("MemoryLifecycle: session-end daily reflection complete.")
        except Exception as e:
            logger.error(f"MemoryLifecycle: session-end daily reflection failed: {e}")
            
        try:
            self.replayer.run()
            logger.info("MemoryLifecycle: session-end experience replay complete.")
        except Exception as e:
            logger.error(f"MemoryLifecycle: session-end experience replay failed: {e}")
