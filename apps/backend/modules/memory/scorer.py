...
class MemoryScorer:
    ...
    def classify_type(self, content: str) -> str:
        ...
        if re.search(r"\bhow to\b|\bguide\b|\btutorial\b", text):
            return "procedural"
        if re.search(r"\bstep-by-step\b|\bsteps to\b|\bworkflow plan\b|\bsetup process\b|\binstall(ation)? process\b", text):
            return "procedural"

        # Episodic: past-tense events
        if re.search(
            r"\byesterday\b|\blast week\b|\blast month\b"
            r"|\bi (fixed|built|created|completed|finished|solved|deployed|did|went|had|saw|got|made|ran|recently)\b"
            r"|\bi used to\b",
            text
        ):
            return "episodic"
        ...
        if re.search(
            r"\bcurrent (goal|plan|task|session|step|tab|view|file|directory|folder|command|status|execution)\b"
            r"|\bcurrently (executing|running|active)\b"
            r"|\bright now\b|\bthis session\b"
            r"|\bactive (plan|task|goal|session|tab)\b|\bopen tab\b|\bclipboard\b|\bplanning\b",
            text
        ):
            return "working"
