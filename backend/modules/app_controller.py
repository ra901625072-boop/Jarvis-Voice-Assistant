...
class AppController:
    ...
    def close_app(self, app_name: str):
        ...
        # Refresh cache if older than 3 seconds
        if time.time() - self.last_process_cache_time > 3:
            try:
                self.running_process_cache = list(psutil.process_iter(['name']))
                self.last_process_cache_time = time.time()
            except Exception as e:
                logger.warning(f"Failed to refresh process cache: {e}")
                self.running_process_cache = []

        for proc in self.running_process_cache:
            try:
